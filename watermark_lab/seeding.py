...
def seed_for_position(secret: str, previous_token_id: int, position: int = 0,
                      hash_name: str = "sha256") -> int:
    """Derive a stable PRNG seed from the private key and token context."""

    material = f"{secret}:{position}:{previous_token_id}".encode("utf-8")
    digest = hashlib.new(hash_name, material).digest()
    return int.from_bytes(digest[:8], "big", signed=False)


def greenlist(token_count: int, secret: str, previous_token_id: int, position: int = 0,
    ...
    seed = seed_for_position(secret, previous_token_id, position, hash_name)
    random.Random(seed).shuffle(candidates)
    return candidates[:count]


def is_green(token_id: int, token_count: int, secret: str, previous_token_id: int,
             position: int = 0, ratio: float = 0.5, hash_name: str = "sha256") -> bool:
    return token_id in greenlist(token_count, secret, previous_token_id, position, ratio, hash_name)
